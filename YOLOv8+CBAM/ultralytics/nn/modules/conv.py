...
__all__ = ('Conv', 'Conv2', 'LightConv', 'DWConv', 'DWConvTranspose2d', 'ConvTranspose', 'Focus', 'GhostConv',
           'ChannelAttention', 'SpatialAttention', 'CBAM', 'Concat', 'RepConv', 'ResNeXtBottleneck')
...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, kernel_size=7):
        ...
        super().__init__()
        print(f"Spartial Attention Kernel Size: {kernel_size}")
...
class ResNeXtBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, cardinality=32, base_width=4, widen_factor=4):
        ...
        # self.cbam_reduce = CBAM(D)

        self.conv_conv = nn.Conv2d(D, D, kernel_size=3, stride=stride, padding=1, groups=cardinality, bias=False)
        self.bn = nn.BatchNorm2d(D)
        # self.cbam_conv = CBAM(D)

        self.conv_expand = nn.Conv2d(D, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn_expand = nn.BatchNorm2d(out_channels)
        self.cbam_out = CBAM(out_channels)
    ...
    def forward(self, x):
        ...
        return F.relu(residual + self.cbam_out(bottleneck), inplace=True)